...
(select empid, max(orderdate) as maxorderdate from sales.Orders group by empid) as c
on o.empid = c.empid and o.orderdate = c.maxorderdate; 
----------------------------------------------						
SELECT O.empid, O.orderdate, O.orderid, O.custid
...
select o.empid, year(o.orderdate) as orderyear, sum(od.qty) as sumQty
 ...
 go
 --order by empid, year(o.orderdate)
...
select v.empid, v.orderyear, v.sumqty, sum(v.sumqty)
